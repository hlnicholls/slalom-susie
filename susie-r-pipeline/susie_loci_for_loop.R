...
for (i in 1:nrow(lead_snps)) {
  ...
  print('After allele flipping:')
  print(target)
  print('sumstat dim:')
  print(dim(sumstat_flip))
  print('ld matrix dim:')
  ...
  df <- merge(sumstat_flip, merged, by='ID', all.x=TRUE)
  print('After getting R2 column:')
  print(target)
  print('sumstat dim:')
  print(dim(df))
  print('ld matrix dim:')
